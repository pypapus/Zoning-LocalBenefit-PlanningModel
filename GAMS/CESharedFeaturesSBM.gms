...
$include EDAndUCSharedFeaturesSBM.gms
...
*MINIMUM FRACTION OF RE GENERATION
                 pREGenAsFracOfDemand
                 pCleanGenAsFracOfDemand
...
$load pCostWeight, pBenefitWeight, pSolarMaxCap
$load pNMaxWind, pNMaxSolar, pNMaxNuclear, pNMaxCC, pNMaxCCS, pNMaxDACS, pPMaxSto, pEMaxSto, pReGenAsFracOfDemand, pCleanGenAsFracOfDemand, pSolarMinCap, pWindMinCap, pCapactech, pHrtech, pOpcosttech
...
*Maximum renewable fraction of demand
         lowerBoundREGenAsFracOfDemand
         lowerBoundCleanGenAsFracOfDemand

*Minimum renewable fraction of demand for each state (Minimumstate RPS)
...
investmentcost..         vFixedcostannual =e= sum(nonstoragetech,vN(nonstoragetech)*pCapactech(nonstoragetech)*(pFom(nonstoragetech)+pOcc(nonstoragetech)*pCrf(nonstoragetech)))
                                                 + sum(storagetech,vPowBuiltSto(storagetech)*pPowOcc(storagetech)*pCrf(storagetech)+vEneBuiltSto(storagetech)*pEneOcc(storagetech)*pCrf(storagetech))
                                                                                                 + sum(l,vNl(l)*pLinecost(l)*pCrfline)+ sum(renewtech,vN(renewtech)* pCapactech(renewtech)*pTxc(renewtech));
* Removed transmission cost                                                                                                 
*+ sum(renewtech,vN(renewtech)*pCapactech(renewtech)*pTxc(renewtech));
***************************************************

*****************CALCULATE LOCAL BENEFITS (VALUE ADDED)*******************
economicbenefits..       vFixedbenefitsannual =e= sum(solartech, vN(solartech)*(pCapactech(solartech)*((pOccValue(solartech)*pCrf(solartech)) + pFomValue(solartech) + (pDecomValue(solartech)*pCrf(solartech)))));
***************************************************
...
lowerBoundREGenAsFracOfDemand.. sum((solaregu,h),vGen(solaregu,h)) + sum((solartech,h),vGentech(solartech,h))
                                    + sum((windegu,h),vGen(windegu,h)) + sum((windtech,h),vGentech(windtech,h))  =g= sum((z,h),pReGenAsFracOfDemand * pDemand(z,h));
                                    

lowerBoundCleanGenAsFracOfDemand.. sum((solaregu,h),vGen(solaregu,h)) + sum((solartech,h),vGentech(solartech,h))
                                    + sum((windegu,h),vGen(windegu,h)) + sum((windtech,h),vGentech(windtech,h))
                                    + sum((CCSegu,h),vGen(CCSegu,h)) + sum((CCStech,h),vGentech(CCStech,h))
                                    + sum((nuclearegu,h),vGen(nuclearegu,h)) + sum((nucleartech,h),vGentech(nucleartech,h)) =g= sum((z,h),pCleanGenAsFracOfDemand * pDemand(z,h));
***************************************************
