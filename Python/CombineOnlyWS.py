#Papa Yaw
#Combine only wind and solar units in fleet together (separately for
#each plant type), then remove all but combined unit from fleet. 
#Combine by subdivisions

#Inputs: gen fleet (2d list)
def combineOnlyWS(genFleet):
    for r in genFleet['GEOID'].unique():
...
def combinePlantsByRegion(fleet,paramCombinedOn,fuelType,r,storage=False):
    gens = fleet.loc[(fleet[paramCombinedOn]==fuelType) & (fleet['GEOID']==r)]
    if gens.shape[0] > 0:
