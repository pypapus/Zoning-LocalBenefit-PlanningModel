...
# scenario_key = "scenario21e" 
############################################################################################################################################################################
## FUNCTIONS TO APPLY LAND-USE TYPE TO EACH SUBDIVISION
############################################################################################################################################################################
def getLanduse():
    Landforms = os.path.join('Data','Parcel_data','all_land_forms.csv')
    Landforms = pd.read_csv(Landforms, index_col='GEOID')
    urbanAreas = Landforms['urbanized_area']
    percentage_cultivated = Landforms['percentage_cultivated']
    percentage_uncultivated = Landforms['percentage_uncultivated']
    percentage_total_cover = Landforms['percentage_total_cover']
    return urbanAreas,percentage_cultivated,percentage_uncultivated,percentage_total_cover


#Function to get rural land (area in sq.m) form subdivisions 
def getRuralLand(subdivAreas):
    urbanAreas,percentage_cultivated,percentage_uncultivated,percentage_total_cover = getLanduse()
    rural_areas = subdivAreas.set_index('GEOID').join(urbanAreas)
    rural_areas = rural_areas.fillna(0)
    rural_areas['subdiv_area'] = rural_areas['subdiv_area'] - rural_areas['urbanized_area']
    rural_areas = rural_areas[rural_areas['subdiv_area'] >= 0] #remove negative values to filter out subdivisions with no land for solar
    rural_areas = rural_areas.drop(['urbanized_area'], axis=1).reset_index()
    return rural_areas
...
def Landuse_cultivated(rural_areas, percentage_cultivated):
        rural_areas = rural_areas.set_index('GEOID').join(percentage_cultivated/100)
        rural_areas = rural_areas.fillna(0)
        rural_areas['subdiv_area'] = rural_areas['subdiv_area'] * rural_areas['percentage_cultivated']
        return rural_areas.drop(['percentage_cultivated'], axis=1)

def Landuse_uncultivated(rural_areas, percentage_uncultivated):
        rural_areas = rural_areas.set_index('GEOID').join(percentage_uncultivated/100)
        rural_areas = rural_areas.fillna(0)
        rural_areas['subdiv_area'] = rural_areas['subdiv_area'] * rural_areas['percentage_uncultivated']
        return rural_areas.drop(['percentage_uncultivated'], axis=1)

def Landuse_both(rural_areas, percentage_total_cover):
        rural_areas = rural_areas.set_index('GEOID').join(percentage_total_cover/100)
        rural_areas = rural_areas.fillna(0)
        rural_areas['subdiv_area'] = rural_areas['subdiv_area'] * rural_areas['percentage_total_cover']
        return rural_areas.drop(['percentage_total_cover'], axis=1)
...
def getSubdivLandcover(Land_use, rural_areas, percentage_cultivated, percentage_uncultivated, percentage_total_cover):
    if Land_use == 'cultivated':
        return Landuse_cultivated(rural_areas, percentage_cultivated)
    elif Land_use == 'uncultivated':
        return Landuse_uncultivated(rural_areas, percentage_uncultivated)
    elif Land_use == 'both':
        return Landuse_both(rural_areas, percentage_total_cover)
    elif Land_use == 'total_cover':
        return rural_areas
...
def getSubdivByZoningPrincipalTownshipsAg(zoningData, reTypeScenario,entry1,entry2,entry3,entry4):
    ...
    zoningData = zoningData.reset_index()
    return zoningData
...
def scenario1(reTypeScenario,Land_use, zoningData, parcels, regions, jurisdiction):
    ...
    subdivAreas = parcels.groupby(
        ['GEOID','CNTY_GEOID','STATEFP','PCA_Code','COUNSUB_LAT','COUNSUB_LON','CNTY_LAT','CNTY_LON'])["Shape_Area"].sum().reset_index()
    subdivAreas = subdivAreas.rename(columns={'Shape_Area':'subdiv_area'})
    rural_areas = getRuralLand(subdivAreas)
    urbanAreas,percentage_cultivated,percentage_uncultivated,percentage_total_cover =  getLanduse()
    rural_areas = getSubdivLandcover(Land_use,rural_areas,percentage_cultivated,percentage_uncultivated,percentage_total_cover)
    #remove negative values from rural areas
    rural_areas = rural_areas.loc[(rural_areas['subdiv_area'] > 0)]
    return rural_areas
...
def scenario2(reTypeScenario,Land_use, zoningData, parcels, regions, jurisdiction):
    ...
    subdivAreas = AllParcelDim.groupby(['GEOID','CNTY_GEOID','STATEFP','PCA_Code','COUNSUB_LAT','COUNSUB_LON','CNTY_LAT','CNTY_LON'])\
        .agg(subdiv_area=('Shape_Area', 'sum'), largest_parcel_area=('Shape_Area', 'max')).reset_index()
    urbanAreas,percentage_cultivated,percentage_uncultivated,percentage_total_cover =  getLanduse()
    rural_areas = getRuralLand(subdivAreas)
    rural_areas = getSubdivLandcover(Land_use,rural_areas,percentage_cultivated,percentage_uncultivated,percentage_total_cover)
    rural_areas = rural_areas.loc[(rural_areas['subdiv_area'] > 0)] 
...
#Scenario 9e: Applying Setbacks - maximum lot size
...
#Scenario 3e: Applying Setbacks - maximum lot size
...
def scenario13(reTypeScenario,Land_use, zoningData, parcels, regions, jurisdiction):
    ...
    if jurisdiction == 'subdivision':
        ...
        data = data.drop(columns=['CNTY_GEOID', 'CNTY_LAT', 'CNTY_LON']).reset_index()
...
#Scenario 20: Zoned, Principal-Use=No, Split to Yes
...
#Scenario 19e: Zone, Principal-Use=No, Split to Yes, Road Setback
...
def scenario21(reTypeScenario,Land_use, zoningData, parcels, regions, jurisdiction):
    data1 = scenario19(reTypeScenario,Land_use, zoningData, parcels, regions, jurisdiction)#unzoned
...
#scenario where we take only outright bans from baseline/progressive
...
scenarios = {
    ...
    ################################################################################################
    "scenario13": scenario13,               #unzoned + zoned 
    ...
    "scenario18e": scenario18e,               #Unzoned + yes in ag district, Road = Yes, PPL = Yes, NPPL = Yes, MinLS = Yes, MaxLS = Yes
    ...
    "scenario21e": scenario21e,              #Zoned, Scenario 19e + Scenario 18e  (All ordinances)
