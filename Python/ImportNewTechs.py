...
def getNewTechs(regElig,regCostFrac,currYear,stoInCE,seasStoInCE,fuelPrices,yearIncDACS,
                incNuc,transRegions,contFlexInelig,ira,lbToShortTon,onlyNSPSUnits=True,allowCoalWithoutCCS=False,
                firstYearForCCS=2030,newPlantDataDir=os.path.join('Data','NewPlantData'),
                iraITCTechs = ['Battery Storage','Hydrogen','Solar PV', 'Wind'], iraPTCTechs=['Nuclear'],
    ...
    #save newtechs to csv for debugging
    
    #Modify costs per IRA (https://www.whitehouse.gov/cleanenergy/clean-energy-tax-provisions/)
    if ira: 
        newTechsCE.loc[newTechsCE['PlantType'].isin(iraITCTechs),'CAPEX(2012$/MW)'] *= 0.7 #apply 30% ITC - assumes meet labor requirements
