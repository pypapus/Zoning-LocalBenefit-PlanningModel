...
def setKeyParameters(interconn):
    ...
    # stateCap = {
    # "IL": {2020: 10000, 2025: 4.364, 2030: 1.151, 2035: 1.089, 2040: 7.991},
    # "IN": {2020: 10000, 2025: 2.725, 2030: 0.776, 2035: 1.650, 2040: 1.791},
    # "MI": {2020: 10000, 2025: 1.812, 2030: 4.885, 2035: 2.677, 2040: 0.677},
    # "MN": {2020: 10000, 2025: 0.178, 2030: 2.436, 2035: 0.268, 2040: 1.917},
    # "OH": {2020: 10000, 2025: 2.337, 2030: 4.519, 2035: 2.747, 2040: 0.322},
    # "WI": {2020: 10000, 2025: 0.591, 2030: 4.428, 2035: 1.266, 2040: 0.302}
    # }
    ...
    retirementCFCutoff = 0.7                            # retire units w/ CF lower than given value
    ...
    initialsolarFracOfDemand = 0                    
    initialwindFracOfDemand = 0   
    ...
    return (buildLimitsCase, greenField, includeRes, useCO2Price, runCE, ceOps, stoInCE, seasStoInCE, ucOrED, numBlocks,
            ...
            initialsolarFracOfDemand, initialwindFracOfDemand, solarMinBuildLoc, windMinBuildLoc, maxCapPerTech,
...
def runMacroCEM(interconn,co2EmsInFinalYear,solarGenFracOfDemand,windGenFracOfDemand,scenario_key,economic_impacts, eim_measure, tx_distance, capex_discount_rate, costWgt=None):
    ...
    for currYear in range(startYear, endYear, yearStepCE):
        ...
        currSolarGenFracOfDemand = initialsolarFracOfDemand + solarGenFracOfDemand/(endYear-1 - startYear) * (currYear - startYear)
        currWindGenFracOfDemand = initialwindFracOfDemand + windGenFracOfDemand/(endYear-1 - startYear) * (currYear - startYear)
        ...
        print('Entering year ', currYear, ' with CO2 cap (million tons):', round(currCo2Cap/1e6),'\t and solar & wind requirement (%):',round(currSolarGenFracOfDemand),round(currWindGenFracOfDemand))
        ...
        if currYear > startYear and runCE:
            ...
            (genFleet, genFleetPriorCE, lineLimits,
             priorCEModel, priorHoursCE) = runCapacityExpansion(genFleet, demandProfile, startYear, endYear, yearStepCE, currYear, planningReserveMargin,
                                                                discountRate, fuelPrices, currCo2Cap,currSolarGenFracOfDemand,
                                                                currWindGenFracOfDemand, solarMinBuildLoc, windMinBuildLoc, numBlocks, daysPerBlock, daysPerPeak,
...
def runCapacityExpansion(genFleet, demand, startYear, endYear, yearStepCE, currYear, planningReserveMargin, discountRate, fuelPrices, 
                         currCo2Cap,currSolarGenFracOfDemand,currWindGenFracOfDemand, solarMinBuildLoc, windMinBuildLoc, numBlocks,
    ...
    pd.Series(currSolarGenFracOfDemand).to_csv(os.path.join(resultsDir,'solarMinGenCE' + str(currYear) + '.csv'))
    pd.Series(currWindGenFracOfDemand).to_csv(os.path.join(resultsDir,'windMinGenCE' + str(currYear) + '.csv'))
    pd.Series(
    {
    ...
    stoTechSet, stoTechSymbols = ceSharedFeatures(db, peakDemandHour, genFleetForCE, newTechsCE, planningReserve, discountRate, currCo2Cap,
                                                  currSolarGenFracOfDemand, currWindGenFracOfDemand,solarMinBuildLoc, windMinBuildLoc, hourSet, hourSymbols, newCfsCE, maxCapPerTech, 
...
def ceSharedFeatures(db, peakDemandHour, genFleet, newTechs, planningReserve, discountRate,
        co2Cap,currSolarGenFracOfDemand,currWindGenFracOfDemand, solarMinBuildLoc, windMinBuildLoc, hourSet, hourSymbols, newCfs, maxCapPerTech, regUpInc, flexInc, stoMkts, 
    ...
    addSolarMinGen(db, currSolarGenFracOfDemand)
    addWindMinGen(db, currWindGenFracOfDemand)
